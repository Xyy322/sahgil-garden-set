...
export function NotificationBell({ userId }: NotificationBellProps) {
  ...
  const resolvePath = (type: string, statusRefId: string) => {
    ...
    if (type === "inquiry") return "/dashboard/customer";
    return "/dashboard/customer";
  };

  const handleNotificationClick = async (id: string, type: string, statusRefId: string) => {
    await markAsRead(id);
    setOpen(false);
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-96 max-w-[92vw] bg-white border border-stone-200 rounded-xl shadow-xl z-50 animate-fade-in">
          <div className="flex items-center justify-between p-4 border-b border-stone-100">
            <p className="font-semibold text-stone-900 text-base">Notifications</p>
            <Button
              size="sm"
              variant="outline"
          ...
          <div className="max-h-96 overflow-y-auto divide-y divide-stone-100">
            {loading ? (
              <div className="p-5 text-sm text-stone-500">Loading notifications...</div>
            ) : latestNotifications.length === 0 ? (
              <div className="p-5 text-sm text-stone-500">No notifications yet.</div>
            ) : (
              latestNotifications.map((notification) => (
                ...
                >
                  <div className="flex flex-col gap-0.5">
                    <span className="text-sm font-semibold truncate">{notification.title || "Notification"}</span>
                    <span className="text-xs text-stone-600 mt-0.5 truncate">{notification.message || "You have a new update."}</span>
                  </div>
                </button>
              ))
            )}
